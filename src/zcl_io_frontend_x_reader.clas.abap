...
"!
CLASS zcl_io_frontend_x_reader DEFINITION
  PUBLIC
...
*    DATA m_str TYPE xstring .
    TYPES x100 TYPE x LENGTH 100.
    DATA lt_buffer TYPE TABLE OF x100.
    DATA aio_itab_x_reader TYPE REF TO zcl_io_itab_x_reader .
...
CLASS zcl_io_frontend_x_reader IMPLEMENTATION.
...
*       header                  = me->header
    ...
    ENDIF.
    CREATE OBJECT aio_itab_x_reader
      EXPORTING
        itab   = lt_buffer
        length = l_filelength.
  ...
  METHOD data_available_internal.
    ...
    available = aio_itab_x_reader->data_available_internal( ).
  ...
  METHOD read_internal.
    result = aio_itab_x_reader->read_internal( length ).
  ENDMETHOD.
